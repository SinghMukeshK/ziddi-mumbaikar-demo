...
export default function NgoServices() {
    ...
    const [generalFundraiser, setGeneralFundraiser] = useState<Fundraiser | null>(null)
    ...
    useEffect(() => {
        ...
        const fetchGeneralFundraiser = async () => {
            try {
                const response = await fundraiserService.getFundraisers({ limit: 1 })
                if (response.success && response.data.length > 0) {
                    setGeneralFundraiser(response.data[0])
                }
            } catch (err) {
                console.error('Failed to fetch general fundraiser:', err)
            }
        }

        fetchServices()
        fetchGeneralFundraiser()
    }, [])
    ...
    const handleDonation = async () => {
        if (!donationAmount || !generalFundraiser) return

        setIsDonating(true)
        try {
            // Step 1: Create Razorpay order on backend
            const orderResponse = await donationService.createRazorpayOrder({
                fundraiser_id: generalFundraiser.id,
                                ...
                                <AnimatePresence mode="wait">
                                    {!donationSuccess ? (
                                            ...
                                            <div className="bg-navy-50 rounded-[2.5rem] p-8 border border-navy-100 relative overflow-hidden group">
                                                ...
                                                <div className="flex flex-col gap-3">
                                                    ...
                                                    <button
                                                        type="button"
                                                        onClick={closeModal}
                                                        className="text-gray-400 font-bold text-sm hover:text-navy-900 transition-colors"
                                                    >
                                                        Skip for Now
                                                    </button>
                                                </div>
                                        ...
                                        >
                                            ...
                                            <h3 className="text-3xl font-black text-navy-900 mb-4">Big Heart, Big Change!</h3>
